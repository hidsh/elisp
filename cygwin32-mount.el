...
;;; modified Aug 2001 by Fred Yankowski <fred@ontosys.com>
;;;  to avoid conflicts with both ange-ftp and Windows UNC pathnames.
    ...
    (save-excursion
      ...
      (while (search-forward-regexp
        ...
        (let ((device (buffer-substring (match-beginning 1)
                                        (match-end 1)))
              (direct (file-name-as-directory (buffer-substring (match-beginning 3)
                                                                (match-end 3)))))
...
;; Cygwin filename pattern rules:
;; + Starts with "/".
;; + Second character, if any, must not be "/" (to avoid conflict with
;;   Windows UNC paths).
;; + All characters from initial "/" to next "/" or end of buffer must
;;   not be ":" or "@" (to avoid conflict with ange-ftp).
;;
(let ((cygwin-pattern "^/\\($\\|[^/:@]+\\($\\|/\\)\\)"))
  (or (assoc cygwin-pattern file-name-handler-alist)
      (setq file-name-handler-alist
            (cons `(,cygwin-pattern . cygwin32-mount-name-hook-function)
