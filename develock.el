...
;; Copyright (C) 2001, 2002, 2003 Katsumi Yamaoka
...
;; that the value is 72.  If too much highlighting offends your eyes,
;; you can change the behavior of highlighting as shown below:
;;
...
(defconst develock-version "0.23"
...
(defcustom develock-mode-strings '(" Develock" " Font")
  "List of identifications to put in the modeline whether Develock mode
is on or off.  Each element should be a string beginning with a space,
a null string or a symbol with a similar string as its value.  You can
use faces for each element if you are using Emacs 21 as follows:
...
(defface develock-bad-manner-face
  ...
  "Develock face used to highlight characters (or words) which violate
the manners of mail messages or news articles."
  :group 'develock
  :group 'font-lock-highlighting-faces
  :group 'font-lock-faces)
...
(defface develock-reachable-mail-address-face
  ...
  "Develock face used to highlight reachable E-mail addresses.  That
would be defenseless to SPAMMERS.")
...
(defun develock-custom-set-and-refontify (symbol value)
  "Function used to the :set keyword for the Develock options which
affect buffer's representation."
  (prog1
      (if (fboundp 'custom-set-default)
	  (custom-set-default symbol value)
	(set-default symbol value))
...
(defcustom develock-max-column-plist
  (list 'emacs-lisp-mode 79
	...
	'message-mode t
	'cmail-mail-mode t)
  "Plist of `major-mode's and limitations that lines should not be longer
than the value.  Value `w' means one subtracted from the window width.
Value t means the return value of `current-fill-column'.  You can
inhibit to highlight long lines using the value nil."
  :type '(repeat
...
(defvar change-log-font-lock-keywords-x nil
  "Internal variable used to have extraordinary level font-lock keywords
for the Change Log mode.")

(defvar texinfo-font-lock-keywords-x nil
  "Internal variable used to have extraordinary level font-lock keywords
for the TeXinfo mode.")
...
(defvar develock-keywords-alist
  '((emacs-lisp-mode lisp-font-lock-keywords-x
    ...
    (cmail-mail-mode cmail-font-lock-keywords-x
		     develock-mail-font-lock-keywords))
  "*Alist of keyword symbols for major modes.  Each element should be
triple symbols of the following form:
...
(defvar develock-inhibit-advice-font-lock-set-defaults nil
  "Internal variable used to inhibit running the advised function
definition of `font-lock-set-defaults' or `font-lock-set-defaults-1'
if it is non-nil.  It will be set to t when the function is called
recursively by itself.  Users should never modify the value.")
...
(defmacro develock-max-column ()
  "Return nil or a number to limit that lines should not be longer than
the value."
...
(defun develock-find-long-lines (limit)
  "Function used to find a long line between the point and LIMIT.  If a
long line is found, it returns non-nil and sets the `match-data' #0,
#1 and #2 with the beginning and the end positions to be highlighted.
This function is affected by the values of `develock-max-column-plist',
...
(defun develock-mode (arg)
  "Toggle Develock mode.  With arg, turn Develock on if and only if arg
is positive.  In Develock mode, highlighting leading and trailing
whitespace, long lines and oddities."
  (interactive "P")
...
(defcustom develock-energize-functions-plist
  ...
  "Plist of functions and flags whether to be advised to remove useless
whitespace."
  :type
  '(list
    :convert-widget
